...
VirusDefender::VirusDefender(){}
VirusDefender::~VirusDefender(){}

void VirusDefender::addLink(int from, int to) {
	auto link = new Link{ from, to };
	this->getNode(from)->links.push_back(link);
	this->getNode(to)->links.push_back(link);
}

void VirusDefender::setGatewayTo(int index) {
	this->gateways.push_back(index);
}

string VirusDefender::setVirusTo(int index)
{
	this->clearNodes();
...
VirusDefender::Node* VirusDefender::getNode(int const index) {
	if (this->network.find(index) == this->network.end())
		this->network[index] = new Node{};
	return this->network[index];
}

VirusDefender::Node* VirusDefender::getNeighbor(Node* const node, int linkIndex) {
...
VirusDefender::Link* VirusDefender::findGatewayLink(int gatewayIndex, int virusIndex, int& length)
{
	this->clearNodes();
...
bool VirusDefender::isGatewayNeighbor(int nodeIndex, int ignore)
{
